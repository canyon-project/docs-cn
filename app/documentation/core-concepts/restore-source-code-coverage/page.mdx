...
前端应用程序编译形态各异，很多情况下是经过预编译后的 AST 交由 babel 进行转换，这样导致覆盖率无法直接映射到源码上，因此我们需要还原源码覆盖率。
...
但是在某些情况下，您可能需要手动获取 sourceMap 文件。
...
| 分类              | devtool                       | 是否能定位到错误源文件   | 报错代码是否与源代码完全一致 | 描述                                    |
|-------------------|-------------------------------|--------------------------|-----------------------------|-----------------------------------------|
...
此时您需要使用到[分离hit和map](core-concepts/separate-hit-and-map)，通过`canyon-uploader`在构建阶段侦测本地的sourceMap文件，Canyon会将他们与覆盖率初始数据匹配上传。
